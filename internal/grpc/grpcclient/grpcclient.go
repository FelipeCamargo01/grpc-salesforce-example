...
func (c *PubSubClient) GetTopic() (*proto.TopicInfo, error) {
	...
	ctx, cancelFn := context.WithTimeout(c.getAuthContext(), config.GRPCCallTimeout)
	defer cancelFn()
...
func (c *PubSubClient) Subscribe(replayPreset proto.ReplayPreset, replayId []byte) ([]byte, error) {
	...
	for {
		...
		for _, event := range resp.Events {
			...
			requestedEvents--
			if requestedEvents < config.Appetite {
...
func (c *PubSubClient) Publish(schema *proto.SchemaInfo) error {
	...
	if result == nil {
		return fmt.Errorf("nil result returned when publishing to %s", config.TopicName)
	}
...
func NewGRPCClient() (*PubSubClient, error) {
	...
	if config.GRPCEndpoint == "localhost:7011" {
	...
	ctx, cancelFn := context.WithTimeout(context.Background(), config.GRPCDialTimeout)
	defer cancelFn()

	conn, err := grpc.DialContext(ctx, config.GRPCEndpoint, dialOpts...)
	if err != nil {
		return nil, err
	}
